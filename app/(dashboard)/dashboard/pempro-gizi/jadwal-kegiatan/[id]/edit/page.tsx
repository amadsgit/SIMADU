...
export default function EditKegiatanPage() {
  ...
  useEffect(() => {
    const fetchKegiatan = async () => {
      try {
        const res = await fetch(`/api/pemproGizi/jadwal/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/pemproGiziJadwal/${id}`, {
        method: 'PUT',
      ...
      <div className="max-w-4xl mx-auto">
        <div className="p-6 border bg-white shadow-md rounded-xl">
          <h1 className="text-2xl font-bold mb-8">
            Edit <span className="text-emerald-700">Kegiatan Program Gizi</span>
          </h1>
