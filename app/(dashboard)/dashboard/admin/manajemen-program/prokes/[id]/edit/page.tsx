...
export default function Page() {
  ...
  const prokesId = params.id; // id dari route /edit/[id]
  ...
  // Fetch data program kesehatan berdasarkan ID
  useEffect(() => {
    const fetchProkes = async () => {
      try {
        ...
        setFormData({
          ...
          klasterId: data.klaster?.id.toString() || '',
        });
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Gagal memperbarui program kesehatan');

      toast.success(data.message || 'Program kesehatan berhasil diperbarui!');
      router.push('/dashboard/admin/manajemen-program/prokes');
    } catch (err: any) {
      console.error(err);
      toast.error(err.message || 'Terjadi kesalahan saat memperbarui program kesehatan!');
    } finally {
      setLoading(false);
    }
  };
