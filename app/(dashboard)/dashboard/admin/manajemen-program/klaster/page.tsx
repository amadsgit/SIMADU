...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        toast.error('Gagal memuat data klaster!');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(result?.error || 'Gagal menghapus klaster!');
        return;
      }

      setKlasterList((prev) => prev.filter((k) => k.id !== selectedId));
      toast.success(result?.message || 'Klaster berhasil dihapus!');
    } catch (error) {
      console.error('Gagal hapus klaster:', error);
      toast.error('Terjadi kesalahan saat menghapus klaster!');
    } finally {
      setShowModal(false);
      setSelectedId(null);
    }
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-100 text-gray-600 uppercase text-xs tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left">Nama Klaster</th>
