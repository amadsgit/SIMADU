...
export default function RekapPage() {
  ...
  useEffect(() => {
    const fetchRekap = async () => {
      try {
        const res = await fetch(`/api/kader/riwayatKegiatan/${id}`);
        if (!res.ok) throw new Error('Gagal mengambil data rekap kegiatan');
  ...
  const handlePrintBalita = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Tanggal Pemeriksaan: ${rekap?.tanggalMulai}`, 40, 50);
    doc.text(`Laporan Kegiatan: ${rekap?.namaKegiatan || '-'}`, 40, 65);
    doc.text(`Program Kesehatan: ${rekap?.program || '-'}`, 40, 80);
    doc.text(`Nama Posyandu: ${rekap?.namaPosyandu|| '-'} (${rekap?.wilayah || '-'})`, 40, 95);
  ...
  const handleDownloadBalita = () => {
    ...
    const headerInfo = [
      ...
      [],
      ['Tanggal Pemeriksaan', rekap?.tanggalMulai || '-'],
      ['Laporan Kegiatan', rekap?.namaKegiatan || '-'],
      ['Program Kesehatan', rekap?.program || '-'],
      ['Nama Posyandu', `${rekap?.namaPosyandu || '-'} (${rekap?.wilayah || '-'})`],
  ...
  return (
    <div className="p-2">
      ...
      {/* INFORMASI KEGIATAN */}
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 shadow-sm mb-5">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm text-gray-700">
          <p><strong>Nama Kegiatan:</strong> {rekap.namaKegiatan}</p>
          <p><strong>Program:</strong> {rekap.program}</p>
          <p><strong>Status:</strong> <span
                className={`inline-block mt-1 px-3 py-1 text-xs font-semibold rounded-full ${getBadgeColor(rekap.status)}`}
              >
                {rekap.status}
              </span>
          </p>
          <p><strong>Tanggal Mulai:</strong> {new Date(rekap.tanggalMulai).toLocaleDateString('id-ID')}</p>
          <p><strong>Tanggal Selesai:</strong> {rekap.tanggalSelesai ? new Date(rekap.tanggalSelesai).toLocaleDateString('id-ID') : '-'}</p>
          <p><strong>Posyandu:</strong> {rekap.namaPosyandu}</p>
          <p><strong>Wilayah:</strong> {rekap.wilayah}</p>
          <p><strong>Kelurahan:</strong> {rekap.kelurahan}</p>
          <p><strong>Kader:</strong> {rekap.namaKader}</p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-6">
        <div className="bg-white p-4 border rounded-lg shadow-sm text-center">
          <BabyIcon className="w-6 h-6 text-emerald-600 mx-auto mb-1" />
          <p className="text-sm font-semibold text-emerald-700">Balita Diperiksa</p>
          <p className="text-lg font-bold">{rekap.jumlahBalita}</p>
        </div>
        <div className="bg-white p-4 border rounded-lg shadow-sm text-center">
          <HeartPulseIcon className="w-6 h-6 text-rose-600 mx-auto mb-1" />
          <p className="text-sm font-semibold text-rose-700">Ibu Hamil Diperiksa</p>
          <p className="text-lg font-bold">{rekap.jumlahIbuHamil}</p>
        </div>
        <div className="bg-white p-4 border rounded-lg shadow-sm text-center">
          <ClipboardCheckIcon className="w-6 h-6 text-indigo-600 mx-auto mb-1" />
          <p className="text-sm font-semibold text-indigo-700">Total Pemeriksaan</p>
